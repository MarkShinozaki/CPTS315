...
# Homework #2 - Movie Recommendations via Item-Item collaborative Filtering 
#
...
class Movie:
    def __init__(self, ID=None, title=None, genre=None, imdbid=None, tmbdid=None):
        ...
        self.imdbid = imdbid
        self.tmbdid = tmbdid
        #Movie, Title, genre, imdbid, tmbdid

class User:
    def __init__(self, ID=None):
        self.ID = ID
        self.movieratings = dict() 
...
def read_movies():
    ...
    with open("movies.csv", "r", encoding='utf-8') as moviescsv, open("links.csv", "r", encoding='utf-8') as linkscsv:
        ...
        for l1,l2 in zip(movie_reader,link_reader):
            movies[l1[0]] = Movie(l1[0], l1[1], l1[2].split(sep="|"), l2[1], l2[2]) 
...
def read_movie_ratings():
    ...
    with open("ratings.csv", "r", encoding='utf-8') as movieratingscsv, open("tags.csv", "r", encoding='utf-8') as tagscsv:
        ...
        for l1 in tag_reader:
            if not l1[0] in users: 
                users[l1[0]] = User(l1[0])
            if l1[1] in users[l1[0]].movieratings: 
                if users[l1[0]].movieratings[l1[1]].tags is None: 
...
def calculate_pairs_cos_sim(ratings_matrix):
    ...
    for i in range(1, len(ratings_matrix)): 
        if str(i) in ratings_matrix: 
            # Possibility movie doesn't exist
            temp = list()
